...
object Naive_RAG_Example {
  ...
  private def createAssistant(documentPath: String) = {
    ...
    val document = loadDocument(Utils.toPath(documentPath), documentParser)
    ...
    val contentRetriever = EmbeddingStoreContentRetriever.builder.embeddingStore(embeddingStore).embeddingModel(embeddingModel).maxResults(2).build() // on each interaction we will retrieve the 2 most relevant segments.minScore(0.5)// we want to retrieve segments at least somewhat similar to user query.build
    ...
    AiServices.builder(classOf[Assistant]).chatModel(chatModel)

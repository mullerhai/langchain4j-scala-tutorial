...
object _07_Advanced_RAG_Multiple_Retrievers_Example {
  ...
  private def createAssistant = {
    ...
    val embeddingStore1 = embed(Utils.toPath("documents/miles-of-smiles-terms-of-use.txt"), embeddingModel)
    val contentRetriever1 = EmbeddingStoreContentRetriever.builder.embeddingStore(embeddingStore1).embeddingModel(embeddingModel).maxResults(2).minScore(0.6).build
    // Let's create our second content retriever.
    val embeddingStore2 = embed(Utils.toPath("documents/biography-of-john-doe.txt"), embeddingModel)
    val contentRetriever2 = EmbeddingStoreContentRetriever.builder.embeddingStore(embeddingStore2).embeddingModel(embeddingModel).maxResults(2).minScore(0.6).build
    // Let's create a query router that will route each query to both retrievers.
    val queryRouter = new DefaultQueryRouter(contentRetriever1, contentRetriever2)
